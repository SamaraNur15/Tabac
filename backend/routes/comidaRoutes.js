...
// POST crear (admin y cajero)
router.post('/', authMiddleware, requireRole(['admin', 'cajero']), asyncH(async (req, res) => {
...
// PUT actualizar (admin y cajero)
router.put('/:id', authMiddleware, requireRole(['admin', 'cajero']), asyncH(async (req, res) => {
  const key = req.params.id;
  ...
  const allowed = [
    ...
    'picante','alergenos','etiquetas','stock','stockMinimo','disponible','tiempo_preparacion_min',
...
// DELETE borrar (admin y cajero)
router.delete('/:id', authMiddleware, requireRole(['admin', 'cajero']), asyncH(async (req, res) => {
  const key = req.params.id;
